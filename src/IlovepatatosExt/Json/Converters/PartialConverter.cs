using Oxide.Ext.IlovepatatosExt.Newtonsoft;
    ...
    private T InternalReadJson(JsonReader reader, JsonSerializer serializer, object existingValue)
    {
        if (reader.TokenType != JsonToken.StartObject)
        {
            throw reader.CreateSerializationException(
                $"Failed to read type '{typeof(T).Name}'. Expected object start, got '{reader.TokenType}' <{reader.Value}>");
        }
        ...
        while (reader.TokenType == JsonToken.PropertyName)
        {
            if (reader.Value is string name)
            {
                ...
                {
                    throw reader.CreateSerializationException(
                        $"Failed to read type '{typeof(T).Name}'. Possible loop when reading property '{name}'");
                }
